...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp' or current_trainer == 'DEFEAT'
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_global = self.ln_post(x[:, 0, :])
        if self.proj is not None:
            x_global = x_global @ self.proj
        return x_global

class VisionTransformer_DEFEAT(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_global = self.ln_post(x[:, 0, :])
        # added 
        x_local = self.ln_post(x[:, 1:, :])
        if self.proj is not None:
            x_global = x_global @ self.proj
            x_local = x_local @ self.proj
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer_DEFEAT(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
